...
mouse_props=$(udevadm info --query=property --name "$mouse_real")
mouse_group=$(echo "$mouse_props" | grep '^LIBINPUT_DEVICE_GROUP=' | cut -d= -f2)
mouse_id_path=$(echo "$mouse_props" | grep '^ID_PATH=' | cut -d= -f2)
mouse_id_path_tag=$(echo "$mouse_props" | grep '^ID_PATH_TAG=' | cut -d= -f2)
mouse_dpi=$(echo "$mouse_props" | grep '^MOUSE_DPI=' | cut -d= -f2)
mouse_click_angle=$(echo "$mouse_props" | grep '^MOUSE_WHEEL_CLICK_ANGLE=' | cut -d= -f2)
mouse_serial=$(echo "$mouse_props" | grep '^ID_SERIAL=' | cut -d= -f2)
mouse_serial_short=$(echo "$mouse_props" | grep '^ID_SERIAL_SHORT=' | cut -d= -f2)
mouse_vendor=$(echo "$mouse_props" | grep '^ID_VENDOR_ID=' | cut -d= -f2)
mouse_model=$(echo "$mouse_props" | grep '^ID_MODEL_ID=' | cut -d= -f2)
mouse_seat=$(echo "$mouse_props" | grep '^ID_SEAT=' | cut -d= -f2)
mouse_is_mouse=$(echo "$mouse_props" | grep '^ID_INPUT_MOUSE=' | cut -d= -f2)
...
if [[ -z "$mouse_group" ]]; then
  echo "Warning: could not read LIBINPUT_DEVICE_GROUP; pointer speed may differ." >&2
fi
...
evdev:name:${mouse_name}:phys:${virtual_phys_hwdb}:serial:*noserial*:
  MOUSE_DPI=${mouse_dpi:-1200@1000 *2400@1000 3200@1000 6400@1000}
  MOUSE_WHEEL_CLICK_ANGLE=${mouse_click_angle:-15}
  LIBINPUT_DEVICE_GROUP=${mouse_group:-3/46d/c332:usb-0000:08:00.3-3}
  ID_INPUT_MOUSE=${mouse_is_mouse:-1}
  ID_VENDOR_ID=${mouse_vendor:-0000}
  ID_MODEL_ID=${mouse_model:-0000}
  ID_SEAT=${mouse_seat:-seat0}
HWDB
...
RULE_FILE="/etc/udev/rules.d/90-volume-wheel.rules"
virtual_phys_rule="volume-wheel/${mouse_phys}"
cat <<RULE | sudo tee "$RULE_FILE" >/dev/null
ACTION=="add|change", KERNEL=="event*", SUBSYSTEM=="input", \
  ATTRS{name}=="${mouse_name}", ATTRS{phys}=="${virtual_phys_rule}", \
  ENV{LIBINPUT_DEVICE_GROUP}:="${mouse_group:-3/46d/c332:usb-0000:08:00.3-3}", \
  ENV{ID_PATH}:="${mouse_id_path:-volume-wheel}", \
  ENV{ID_PATH_TAG}:="${mouse_id_path_tag:-volume-wheel}", \
  ENV{MOUSE_DPI}:="${mouse_dpi:-1200@1000 *2400@1000 3200@1000 6400@1000}", \
  ENV{MOUSE_WHEEL_CLICK_ANGLE}:="${mouse_click_angle:-15}", \
  ENV{ID_SERIAL}:="${mouse_serial:-volume-wheel}", \
  ENV{ID_SERIAL_SHORT}:="${mouse_serial_short:-volume-wheel}", \
  ENV{ID_VENDOR_ID}:="${mouse_vendor:-0000}", \
  ENV{ID_MODEL_ID}:="${mouse_model:-0000}", \
  ENV{ID_INPUT_MOUSE}:="${mouse_is_mouse:-1}", \
  ENV{ID_SEAT}:="${mouse_seat:-seat0}"
RULE
...
if ! grep -q '^uinput$' "$MODULES_CONF" 2>/dev/null; then
  ...
  printf 'uinput\n' | sudo tee -a "$MODULES_CONF" >/dev/null
fi
